...
Steps in this analysis are similar to the steps taken in the [line transect analysis of winter wren data](https://distancedevelopment.github.io/Distance/docs/articles/lines-distill.html).
...
The code above overwrites the strata designations in the original data to make it appear that all data were derived from a single stratum.  This makes the analysis simpler to perform.  There are examples of analysis of [stratified data in another vignette](https://distancedevelopment.github.io/Distance/docs/articles/web-only/strata/strata-distill.html).
...
The object `Savannah_sparrow_1980` is a dataframe object made up of rows and columns. In contrast to the [Montrave winter wren line transect data used in the previous vignette](https://distancedevelopment.github.io/Distance/docs/articles/lines-distill.html), Savannah sparrows were not detected at all point transects.  Radial distances receive the value `NA` for transects where there were no detections.  To determine the number of detections in this data set, we total the number of values in the `distance` field that are not `NA`
