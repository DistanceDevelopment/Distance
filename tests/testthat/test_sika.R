...
test_that("10% truncation", {
  ...
  result <- ddf(dsmodel = ~mcds(key = "hn", formula = ~1, adj.series="cos", adj.order=2),
                data = dat, method = "ds", meta.data = list(width = 175),
                control=list(initial=list(adjustment=0.08493070, scale=log(121.1144)),
                             nofit=TRUE))
  df <- result
  ...
  deer_ests <- dht2(df, flatfile=sikadeer,
                    ...
                    convert_units=cu, multipliers=mult, stratification="effort_sum",
                    total_area=13.9)
  ...
  expect_equal(deer_ests$Abundance,
               c(1027.0, 383.00, 34.000, 29.000, 210.00, 126.00, 17.000, 69.000, 497), tol=1e-2)
  expect_equal(deer_ests$Abundance_CV,
               c(18.95, 25.71, 24.24, 47.78, 8.87, 50.39, 8.87, 8.87, 15.79)/100, tol=tol)
  expect_equal(deer_ests$df,
               c(19.18, 11.88, 2.74, 4.26, 47281.58, 2.07, 47281.58, 47281.58, 20.22), tol=tol)
